import argparse
import getpass
import re
import sys
from importlib.metadata import version
from pathlib import Path

from zxtx.constants import CIPHER_METHOD, COMPRESSION_METHOD
from zxtx.dtypes import ZXTXBody, ZXTXData, ZXTXHeader
from zxtx.highlevel import open, write_stream
from zxtx.parser import parse_zxtx_header
from zxtx.signer import load_private_key
from zxtx.utils import error, note
from zxtx.verifier import verify_signature

# Preserve original print for DummyConsole fallback
_print = print

# Regex patterns to clean style tags
STYLE_TAG_PATTERN = re.compile(
    r"""
    (?<!\\)            # not preceded by backslash
    (?<!\[)            # not a double opening bracket
    \[/?[^\[\]]+?\]    # match [style] or [/]
    (?!\])             # not a double closing bracket
    """,
    re.VERBOSE,
)

DOUBLE_BRACKET_PATTERN = re.compile(r"\[\[([^\[\]]+?)\]\]")


def strip_style_tags(text: str) -> str:
    text = DOUBLE_BRACKET_PATTERN.sub(r"[\1]", text)
    text = STYLE_TAG_PATTERN.sub("", text)
    return text.replace("\\[", "[").replace("\\]", "]")


try:
    from rich.console import Console
    from rich.pretty import pprint
    from rich.progress import BarColumn, Progress, TaskProgressColumn, TextColumn

    HAS_RICH = True
except ImportError:
    from pprint import pprint

    HAS_RICH = False


def is_tty():
    """Check if stdout is a TTY (interactive terminal)."""
    return sys.stdout.isatty()


def get_password_interactive() -> bytes | None:
    """Get password from interactive prompt."""
    if not is_tty():
        return None
    try:
        password = getpass.getpass(
            "Enter private key password (or press Enter for none): "
        )
        return password.encode("utf-8") if password else None
    except (KeyboardInterrupt, EOFError):
        return None


def create_progress_bar(description: str = "Processing"):
    """Create a progress bar if rich is available and we're in a TTY."""
    if not HAS_RICH or not is_tty():
        return None

    console = Console(stderr=True)  # Progress to stderr so stdout stays clean
    progress = Progress(
        TextColumn("[progress.description]{task.description}"),
        BarColumn(),
        TaskProgressColumn(),
        console=console,
        transient=True,
    )
    return progress


def get_bytes(path: str | None) -> bytes | None:
    if path:
        return Path(path).read_bytes()
    return None


def load_private_key_with_prompt(
    private_key_path: str | None, password: bytes | None = None
) -> bytes | None:
    """Load private key, prompting for password if needed and not provided."""
    if not private_key_path:
        return None

    key_bytes = Path(private_key_path).read_bytes()

    # Try loading without password first
    try:
        load_private_key(key_bytes, password=None)

        return key_bytes
    except Exception:
        # Key is encrypted, need password
        if password is None:
            password = get_password_interactive()
            if password is None:
                error("Password required for encrypted private key")
                sys.exit(1)

        # Verify password works
        try:
            load_private_key(key_bytes, password=password)
            return key_bytes
        except Exception:
            error("Invalid password for private key")
            sys.exit(1)


def clean_error(e: Exception) -> str:
    return str(e).removeprefix("[Errno 2]").removeprefix("[Errno 13]").lstrip(" ")


def explain_error(e: Exception):
    error_str = str(e)
    error_repr = repr(e)

    if error_str == "Decryption failed":
        note("This may be caused by the following reasons:")
        note("  - The private key is incorrect.")
        note("  - The password of the private key is incorrect.")
        note("  - A wrong cipher or compression method is used,")
        note("    this should be left to the parser to determine.")
    elif error_repr == "InvalidTag()":
        note("This may be caused by the following reasons:")
        note("  - You've provided an invalid cipher method,")
        note("    this should be left to the parser to determine.")


def write_cmd(args):
    password_bytes = args.password.encode("utf-8") if args.password else None

    try:
        # Handle stdin input
        if str(args.input_path) == "-":
            input_data = sys.stdin.buffer.read()
        else:
            input_data = Path(args.input_path).read_bytes()

        # Handle stdout output with progress bar if TTY
        if str(args.output_path) == "-":
            write_stream(
                input_data,
                sys.stdout.buffer,
                cipher=CIPHER_METHOD(args.cipher),
                compression=COMPRESSION_METHOD(args.compression),
                password=password_bytes,
                certificate=get_bytes(args.certificate),
                private_key=get_bytes(args.private_key),
                public_key=get_bytes(args.public_key),
            )
        else:
            # File output with progress
            progress = create_progress_bar("Writing ZXTX file")
            if progress:
                with progress:
                    task = progress.add_task("Writing", total=len(input_data))
                    with open(
                        args.output_path,
                        cipher=CIPHER_METHOD(args.cipher),
                        compression=COMPRESSION_METHOD(args.compression),
                        password=password_bytes,
                        certificate=get_bytes(args.certificate),
                        private_key=get_bytes(args.private_key),
                        public_key=get_bytes(args.public_key),
                    ) as output_file:
                        output_file.write_bytes(input_data)
                    progress.update(task, completed=len(input_data))
            else:
                with open(
                    args.output_path,
                    cipher=CIPHER_METHOD(args.cipher),
                    compression=COMPRESSION_METHOD(args.compression),
                    password=password_bytes,
                    certificate=get_bytes(args.certificate),
                    private_key=get_bytes(args.private_key),
                    public_key=get_bytes(args.public_key),
                ) as output_file:
                    output_file.write_bytes(input_data)

    except Exception as e:
        error(f"{clean_error(e)}")
        sys.exit(1)


def read_cmd(args):
    password_bytes = args.password.encode("utf-8") if args.password else None
    private_key_bytes = load_private_key_with_prompt(args.private_key, password_bytes)

    try:
        # Handle stdin input
        if str(args.input_path) == "-":
            raw_data = sys.stdin.buffer.read()
            header, body = parse_zxtx_header(
                raw_data,
                private_key=private_key_bytes,
                cipher_method=CIPHER_METHOD(args.cipher) if args.cipher else None,
                compression_method=(
                    COMPRESSION_METHOD(args.compression) if args.compression else None
                ),
            )
            data = body.data

            # Verify signature if requested
            if args.verify:
                trusted_cert = get_bytes(args.certificate)
                if not verify_signature(
                    trusted_cert,
                    header.certificate,
                    header.serialize_for_signature(),
                    body.encrypted_payload,
                    header.signature,
                ):
                    error("Signature verification failed")
                    note("This may be caused by the following reasons:")
                    note("  - The file does not have a signature.")
                    note("  - The provided certificate is incorrect or invalid,")
                    note("    or no certificate was provided at all.")
                    note("  - The file or it's signature is invalid.")
                    sys.exit(1)
                note("Signature verified successfully")
        else:
            with open(
                args.input_path,
                password=password_bytes,
                public_key=get_bytes(args.public_key),
                private_key=private_key_bytes,
                certificate=get_bytes(args.certificate),
                cipher=CIPHER_METHOD(args.cipher),
                compression=COMPRESSION_METHOD(args.compression),
            ) as file:
                data = file.read_bytes()

                # Verify signature if requested
                if args.verify:
                    header = file._header
                    body = file._body
                    trusted_cert = get_bytes(args.certificate)
                    if not verify_signature(
                        trusted_cert,
                        header.certificate,
                        header.serialize_for_signature(),
                        body.encrypted_payload,
                        header.signature,
                    ):
                        error("Signature verification failed")
                        note("This may be caused by the following reasons:")
                        note("  - The file does not have a signature.")
                        note("  - The provided certificate is incorrect or invalid,")
                        note("    or no certificate was provided at all.")
                        note("  - The file or it's signature is invalid.")
                        sys.exit(1)
                    note("Signature verified successfully")

        # Handle stdout output or file output
        if args.output_path is None or str(args.output_path) == "-":
            try:
                sys.stdout.buffer.write(data)
            except Exception:
                # Fallback for non-binary stdout
                try:
                    print(data.decode("utf-8"))
                except UnicodeDecodeError:
                    print(repr(data))
        else:
            Path(args.output_path).write_bytes(data)

    except Exception as e:
        error(f"{clean_error(e)}")
        sys.exit(1)


def dump_cmd(args):
    password_bytes = args.password.encode("utf-8") if args.password else None

    try:
        # Handle stdin input
        if str(args.input_path) == "-":
            raw_data = sys.stdin.buffer.read()

            header, body = parse_zxtx_header(
                raw_data,
                private_key=get_bytes(args.private_key),
                cipher_method=CIPHER_METHOD(args.cipher) if args.cipher else None,
                compression_method=(
                    COMPRESSION_METHOD(args.compression) if args.compression else None
                ),
            )
            data = b"<stream: use 'read' command to decrypt data>"
        else:
            with open(
                args.input_path,
                password=password_bytes,
                public_key=get_bytes(args.public_key),
                private_key=get_bytes(args.private_key),
                certificate=get_bytes(args.certificate),
                cipher=CIPHER_METHOD(args.cipher),
                compression=COMPRESSION_METHOD(args.compression),
            ) as zxtx_file:
                header: ZXTXHeader = zxtx_file._header
                body: ZXTXBody = zxtx_file._body
                data: bytes = zxtx_file._data

                zxtx_file._read()

        pprint(header)
        pprint(body)
        pprint(ZXTXData(data=data))

    except Exception as e:
        error_msg = clean_error(e)
        error_repr = repr(e)

        if error_msg.strip():
            error(f"{error_msg}")
        elif error_repr == "InvalidTag()":
            error("[bold magenta]cryptography.exceptions.InvalidTag[/bold magenta]")
        else:
            error(f"{error_repr}")

        explain_error(e)
        sys.exit(1)


def add_common_options(p):
    p.add_argument(
        "-c",
        "--cipher",
        type=int,
        default=0,
        choices=[c.value for c in CIPHER_METHOD],
        help="Cipher method (int).",
    )
    p.add_argument(
        "-z",
        "--compression",
        type=int,
        default=0,
        choices=[c.value for c in COMPRESSION_METHOD],
        help="Compression method (int).",
    )
    p.add_argument(
        "-p",
        "--password",
        type=str,
        help="Password to decrypt private key (if needed).",
    )
    p.add_argument("-k", "--public-key", type=str, help="Path to public key file.")
    p.add_argument("-u", "--private-key", type=str, help="Path to private key file.")
    p.add_argument("-C", "--certificate", type=str, help="Path to certificate file.")

    p.add_argument(
        "-V",
        "--version",
        action="version",
        version=f"zxtx {version('zxtx')}",
        help="Print version and exit",
    )


def main():
    parser = argparse.ArgumentParser(description="ZXTX CLI")
    subparsers = parser.add_subparsers(dest="command")

    parser.add_argument(
        "-V",
        "--version",
        action="version",
        version=f"zxtx {version('zxtx')}",
        help="Print version and exit",
    )

    write_parser = subparsers.add_parser("write", help="Write a ZXTX file.")
    write_parser.add_argument(
        "input_path", type=Path, help="Input file path (use '-' for stdin)."
    )
    write_parser.add_argument(
        "output_path", type=Path, help="Output .zxtx file path (use '-' for stdout)."
    )
    add_common_options(write_parser)
    write_parser.set_defaults(func=write_cmd)

    read_parser = subparsers.add_parser("read", help="Read a ZXTX file.")
    read_parser.add_argument(
        "input_path", type=Path, help="Input .zxtx file path (use '-' for stdin)."
    )
    read_parser.add_argument(
        "output_path",
        type=Path,
        nargs="?",
        help="Where to save extracted output (use '-' or omit for stdout).",
    )
    read_parser.add_argument(
        "--verify",
        action="store_true",
        help="Verify signature after reading (requires -C/--certificate).",
    )
    add_common_options(read_parser)
    read_parser.set_defaults(func=read_cmd)

    dump_parser = subparsers.add_parser("dump", help="Dump ZXTX file metadata.")
    dump_parser.add_argument(
        "input_path", type=Path, help="Input .zxtx file path (use '-' for stdin)."
    )
    add_common_options(dump_parser)
    dump_parser.set_defaults(func=dump_cmd)

    args = parser.parse_args()

    if args.command is None:
        parser.print_help()
        sys.exit(1)

    args.func(args)


if __name__ == "__main__":
    main()
